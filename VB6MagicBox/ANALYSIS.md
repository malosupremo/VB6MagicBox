...
- **Default property expansion**: explicates implicit default property access for controls only on **evident assignment targets** (e.g., `LblName = "Hello"` → `LblName.Caption = "Hello"`)
  ...
  - `ApplyDefaultProperties(line, controlDefaults, ref count)`: **conservative** per-line transformation:
    - Only processes lines where the control is the **assignment target** (first token on the line): `^\s*ControlName\b(\s*\([^)]*\))?\s*=`
    - Handles control arrays: `ControlName(index) = value` → `ControlName(index).Caption = value`
    - Skips `Begin` lines (form designer), `Set` lines (object assignment), empty lines
    - Skips if already followed by `.` (explicit property access)
    - Does NOT add default properties when control is used as function argument (e.g., `GetFieldNumber PnlSteps, ...` — PnlSteps may be passed as object), in comparisons, or in any non-assignment context
...
- **Default property expansion**: TypeAnnotator Phase 3 now explicates implicit default property accesses for VB6 controls, but **only on evident assignment targets** (control is the first token on the line, followed by optional array index and `=`). 21 control types mapped (VB.Label→Caption, VB.TextBox→Text, VB.ComboBox→Text, VB.CheckBox→Value, etc.). Per-module `BuildControlDefaultMap` maps ConventionalName→defaultProperty from Controls list. `ApplyDefaultProperties` uses conservative regex `^\s*ControlName\b(\s*\([^)]*\))?\s*=` to match only assignment targets. Skips `Set`/`Begin` lines and already-explicit `.` access. Does NOT modify controls used as function arguments (e.g., `GetFieldNumber PnlSteps, ...` where the function expects the control object). Separate `totalDefaultProps` counter reported in summary
- **Array return type parsing fix**: `ReFunction`/`ReProperty`/`ReDeclareFunction` regexes use greedy `(.*)` for parameter capture, which consumed `()` from array return types (e.g., `As Integer()`). The last `)` on the line was matched as the parameter-list close, leaving the return type group empty. Fix: added `ExtractParamsAndReturnType(line, openParenPos)` helper in `Parser.Core.Params.cs` that uses balanced parenthesis counting to find the correct closing `)`. Applied as fallback in Function, DeclareFunction, and Property parsing — when the regex yields an empty return type, re-extract using balanced parens
- **VBP extension validation**: `ReadVbpPath` now rejects files that don't have `.vbp` extension before saving the path. `RunAnalysis` (CLI mode) also validates. Prevents accidentally feeding a binary DLL/EXE to the parser, which would produce an "inconcludente" analysis on a non-text file
