...
- **HarmonizeSpacing**: normalizes blank lines; rules: S0 fallback; S14 wins for initial declaration groups; S7 comments before block use blank + comments + block; S8 comments inside block use block start + comments + blank; single-line If always has blank after unless adjacent to If/End If boundaries; Property Get/Let/Set blocks with same name stay adjacent
...
- Enum naming: screaming snake to PascalCase, preserves short acronyms (e.g., `SQM`)
...
- **Duplicate chain resolution fix**: `EnumerateParenContents` re-discovers dot-chains already resolved by the main `EnumerateDotChains` loop (e.g., `Trim$(WFrmRecipe.Recipe.Steps(stepIndex).Deposit.Material)` — the chain inside `Trim$(...)` is found by both loops). On the second resolution, `FindTokenInRawLine` skips the correct raw positions (already claimed) and maps homonym tokens to wrong positions (e.g., the field `Material` at position 74 is claimed, so the second resolution maps it to position 4 — the bare control `Material`). This false field reference blocks the control's reference via `recorded` deduplication. Fix: in `ResolveChain`, before mapping positions, check if the first structural token's natural raw position is already claimed; if so, the chain is a duplicate and is skipped early
